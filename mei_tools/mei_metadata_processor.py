...
class MEI_Metadata_Updater:
    ...
    def apply_metadata(self, mei_path, matching_dict, output_folder):
        ...
        for editor in editors:
            ...
            # The CSV stores editors as "Name [role]" for readability;
            # the role already becomes the @role attribute above, so
            # strip the bracketed annotation from the name text itself.
            name = re.sub(r'\s*\[[^\]]*\]\s*$', '', editor).strip()
            editor_elem.text = name
